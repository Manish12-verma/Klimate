...
const WeatherForecast = ({ data }: WeatherForecastProps ) => {
    ...
    const dailyForecasts = data.list.reduce((acc,forecast)=>{
          const date = format(new Date(forecast.dt*1000),'yyyy-MM-dd');   //converting it to unix timestamp
          if(!acc[date]){
              acc[date] = {
  ...
  <CardContent>
    <div className="grid gap-4">
      {nextDays.map((day)=>{
        return <div className="grid grid-cols-3 items-center gap-4 rounded-lg border p-4" key={day.date}>
